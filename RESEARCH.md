# Mechanome scientific reference

**Reading or changing the code?** Start with [the codebase guide](CODEBASE.md)
for the main call paths, reporting rules, and development checks.
...
importable Python packages at the repository root, with scientific detail in two
references — this `RESEARCH.md` and the manuscript `MANUSCRIPT.md` — and all
figures, results, and reproducibility scripts kept beside the code that
produces them.

```
mechanome/                     ← repository root (the project)
├── README.md                  ← installation and entry points
├── CODEBASE.md                ← code flow and development guide
├── RESEARCH.md                ← this file: scientific detail and reproduction
...
2. **Bayesian inverse** (`inverse.py`) inverts the forward model — the same
   ...
   **active-stress / cortex term** (a cortical force pulling the cap inward, work
   ...
   a possible future approach, not an implemented backend.
...
- **PICALM cannot make a productive pit alone.** Its autonomous ANTH wedge
  ...
  ![PICALM orchestration test case](figures/fig3_picalm_orchestration.png)
...
- **Epsin's two domains are complementary, with a force-burden ordering.** Full
  ...
  ![Epsin domain cases](figures/fig2_epsin_domain_cases.png)
...
Neural posterior inference (SBI) and FreeDTS simulation are future work.
Their unused placeholder APIs have been removed. The MD-gap queue still
records simulation requests and uncertainty; it does not run simulations.
...
| **FreeDTS Tier-1** | **NOT IMPLEMENTED** | A possible future simulation backend. |
| **Averaged clathrin-track imaging data** | **NOT IN HAND** | CCS target anchored to published CCP geometry (R≈30–50 nm ⇒ Ω-stage H≈0.02–0.03 nm⁻¹); an ingestion adapter would be needed for those data |
...
`curvo/md_gap_queue.py` records simulation requests when a parameter does not
cover the target state. Each request identifies the system, observable, and
estimator. The queue retains explicit uncertainty and gap flags; a simulation
...
(requires a simulation backend) on one case to confirm the closed-form ranking
...
The four falsifiable structure-based predictions in this reference
...
![orchestration model](figures/fig6_orchestration.png)
