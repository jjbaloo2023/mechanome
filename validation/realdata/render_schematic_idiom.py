"""
render_schematic_idiom.py -- compose CME decision-tree figures in the 2020 Comm
Bio Fig 7 idiom, for a chosen curvature-generating protein.

Each protein is a folded membrane-binding domain (real secondary-structure
cartoon sprite) plus an intrinsically disordered region drawn as per-molecule
crowding chains anchored to that domain:

    epsin  = ENTH (RCSB 1H0A)                 + IDP  -> compose_figure("enth_cartoon", "epsin", ...)
    AP180  = ANTH (AlphaFold SNAP91 O60641)   + IDP  -> compose_figure("anth_ap180_cartoon", "AP180", ...)

Clathrin is a faceted lattice (shallow-angle straight segments, buckyball-edge
idiom); AP2 is a simple glyph; both are supporting cast.
...
OUT_EPSIN = os.path.join(_HERE, "schematic_idiom_epsin.png")
OUT_AP180 = os.path.join(_HERE, "schematic_idiom_ap180.png")
...
def compose_figure(primary, protein, domain, path, dpi=200):
    """Render the two-panel decision tree for one protein.

    primary  : cartoon sprite name (e.g. 'enth_cartoon', 'anth_ap180_cartoon')
    protein  : protein name for titles (e.g. 'epsin', 'AP180')
    domain   : folded-domain label for the header/legend (e.g. 'ENTH', 'ANTH')
    """
    ...
    def N(ax_, cx, cy, w, curv, n, **kw):
        si.node(ax_, cx, cy, w, curv, n, primary=primary, **kw)
    A = si.branch_arrow
    # TOP: without the protein
    ax.text(0.03, 1.13, f"Coat assembly without {protein}", fontsize=14.5, fontweight="bold", zorder=8)
    ...
    # BOTTOM: with the protein
    ax.text(0.03, 0.60, f"Coat assembly with {protein} recruitment", fontsize=14.5, fontweight="bold", zorder=8)
    ...
    ax.text(0.40, 0.563, f"mediated by {domain} domain", color="white", fontsize=7.4, ha="center", va="center", fontweight="bold", zorder=9)
    ax.text(0.58, 0.563, "mediated by IDP domain", color="white", fontsize=7.4, ha="center", va="center", fontweight="bold", zorder=9)
    N(ax, 0.12, 0.40, 0.13, 0.0, 2); title(0.12, 0.455, f"flat lattice\n+ {protein}")
    lab(0.265, 0.495, f"Low tension favors\nless {protein} nucleation", 7.4)
    lab(0.265, 0.295, f"High tension favors\nmore {protein} nucleation", 7.4)
...
def build_all():
    if not os.path.exists(os.path.join(si.SPRITE_DIR, "enth_cartoon.png")):
        si.build_sprites()
    compose_figure("enth_cartoon", "epsin", "ENTH", OUT_EPSIN)
    compose_figure("anth_ap180_cartoon", "AP180", "ANTH", OUT_AP180)
    return [OUT_EPSIN, OUT_AP180]


# backward-compatible alias
OUT = OUT_EPSIN
def compose(path=OUT_EPSIN, dpi=200):
    return compose_figure("enth_cartoon", "epsin", "ENTH", path, dpi)
