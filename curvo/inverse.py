"""
inverse.py — the Bayesian inverse engine: extracted geometry(t) -> posterior over
mechanical forces, with EVIDENCE for model comparison and a first-class
IDENTIFIABILITY report.

Design (hardware-driven, per the v2 decision record)
-----------------------------------------------------
The forward model is cheap (closed-form spherical-cap energy minimization per
frame), so EXACT inference is tractable on a 4-core box -- no GPU, no amortized
neural posterior needed. We therefore run:

  * NESTED SAMPLING (dynesty) as the PRIMARY engine: it returns both the posterior
    AND the Bayesian evidence log Z, which the mechanism-discrimination core
    (Phase 3) needs to rank competing hypotheses.
  * MCMC (emcee) as an independent cross-check on the posterior.
  * SBI / amortized neural posterior is left as a clean, documented seam
    (fit_sbi stub) for when many-thousand-inference throughput is needed.

Forward model
-------------
theta = (c_eff_max, active_force_max_pN, sigma_kBT_nm2). A clathrin coat assembles
over the movie (coverage ramps sigmoidally), scaling the spontaneous-curvature and
cortical-force actors; the evaluator returns the energy-minimizing cap geometry per
frame. predict_H returns the mean-curvature trajectory H(t) -- the observable the
PerceptionProvider delivers.

Likelihood
----------
Gaussian in H(t) with the PER-FRAME uncertainty the perception layer reports
(unresolved frames carry inflated sigma and are automatically down-weighted). This
is what makes the posterior honest: where the data cannot constrain a force, the
prior shows through, and the identifiability report flags it.

Identifiability
---------------
A force is only "identified" if the data move its posterior appreciably inside the
prior. We quantify this per parameter as the posterior-to-prior width ratio and the
prior->posterior KL; a parameter whose posterior ~ its prior is reported as
UNIDENTIFIABLE. This is the anti-"force-astrology" gate: analyze() must refuse a
confident point value for an unidentifiable actor and return the posterior instead.
"""
...
def _fast_H_trajectory(sigma, c_eff_max, active_max, kappa, coat_rig, A, T,
                       ramp_mid=0.45, ramp_width=0.12):
    ...
    c_eff = c_eff_max * cov                      # (T,)
    active = active_max * cov                    # (T,)
    rig = 1.0 + (coat_rig - 1.0) * cov           # (T,)
    kappa_eff = kappa * rig                       # (T,)
    psi = _PSI[None, :]                           # (1,P)
    omc = np.clip(1.0 - np.cos(psi), 1e-9, None)  # (1,P)
    R = np.sqrt(A / (2 * np.pi * omc))            # (1,P) - psi-only
    depth = R * omc                               # (1,P)
    footprint = np.pi * (R * np.sin(psi)) ** 2    # (1,P)
    # energy terms broadcast over (T,P)
    bending = (kappa_eff[:, None] / 2) * (2.0 / R - c_eff[:, None]) ** 2 * A
    tension = sigma * (A - footprint)
    active_E = -(active[:, None] * depth) / kBT
    E = bending + tension + active_E              # (T,P); line term lam=0
    i = np.argmin(E, axis=1)                       # (T,)
    R_opt = R[0, i]
    return 1.0 / R_opt                             # H(t)


def predict_H(theta, params, A_coat_nm2, fixed=FIXED):
    """Forward model: force vector theta -> mean-curvature trajectory H(t)."""
    d = {p.name: v for p, v in zip(params, theta)}
    return _fast_H_trajectory(
        sigma=d.get("sigma", 0.02), c_eff_max=d.get("c_eff_max", 0.0),
        active_max=d.get("active_force_max", 0.0), kappa=fixed["kappa_kBT"],
        coat_rig=fixed["coat_rigidity_factor"], A=A_coat_nm2, T=fixed["T"])


# ---------------------------------------------------------- likelihood -------
def make_loglike(H_obs, H_sigma, params, A_coat_nm2, fixed=FIXED, mask=None,
                 actin_obs=None, actin_sigma=None):
    ...
    if use_actin:
        a_obs = np.asarray(actin_obs, float)
        a_sig = np.clip(np.asarray(actin_sigma, float), 1e-3, None)
        aidx = np.where(mask)[0]

    def loglike(theta):
        Hm = predict_H(theta, params, A_coat_nm2, fixed)
        r = (Hm[idx] - H_obs[idx]) / H_sigma[idx]
        ll = float(-0.5 * np.sum(r * r) - np.sum(np.log(H_sigma[idx])) -
        ...
        if use_actin:
            ...
            d = {p.name: v for p, v in zip(params, theta)}
            cov = _coverage_ramp(fixed["T"])
            af_frac = d.get("active_force_max", 0.0) / sm.ACTIN_CALIB_PN
            a_pred = cov * af_frac
            ra = (a_pred[aidx] - a_obs[aidx]) / a_sig[aidx]
            ll += float(-0.5 * np.sum(ra * ra) - np.sum(np.log(a_sig[aidx])) -
                        0.5 * len(aidx) * np.log(2 * np.pi))
        return ll
    return loglike
...
def _coverage_ramp(T, ramp_mid=0.45, ramp_width=0.12):
    t = np.linspace(0, 1, T)
    cov = 1.0 / (1.0 + np.exp(-(t - ramp_mid) / ramp_width))
    return (cov - cov.min()) / (cov.max() - cov.min())
...
def run_nested(H_obs, H_sigma, A_coat_nm2, params=None, mask=None,
    ...
    loglike = make_loglike(H_obs, H_sigma, params, A_coat_nm2, fixed, mask,
...
# --------------------------------------------------------------- MCMC --------
def run_mcmc(H_obs, H_sigma, A_coat_nm2, params=None, mask=None,
             nwalkers=32, nsteps=2000, burn=500, seed=0, fixed=FIXED,
             actin_obs=None, actin_sigma=None):
    ...
    rng = np.random.default_rng(seed)
    p0 = np.array([[p.lo + (p.hi - p.lo) * rng.random() for p in params]
                   for _ in range(nwalkers)])
    sampler = emcee.EnsembleSampler(nwalkers, ndim, logpost)
    sampler.run_mcmc(p0, nsteps, progress=False)
    chain = sampler.get_chain(discard=burn, flat=True)
    return dict(samples=chain, params=params, engine="emcee",
                acceptance=float(np.mean(sampler.acceptance_fraction)))


# ------------------------------------------------- identifiability report ----
def identifiability(samples, params, width_ratio_thresh=0.5, corr_thresh=0.7,
                    rail_frac=0.15):
    ...
    """
    n = len(params)
    corr = np.corrcoef(samples.T) if n > 1 else np.array([[1.0]])
    # find strongly-correlated (degenerate) pairs
    degenerate = {p.name: [] for p in params}
    for i in range(n):
        for j in range(i + 1, n):
            if abs(corr[i, j]) > corr_thresh:
                degenerate[params[i].name].append(
                    dict(partner=params[j].name, corr=float(corr[i, j])))
                degenerate[params[j].name].append(
                    dict(partner=params[i].name, corr=float(corr[i, j])))
    report = {}
    for j, p in enumerate(params):
        s = samples[:, j]
        prior_std = (p.hi - p.lo) / np.sqrt(12.0)     # uniform prior std
        post_std = float(np.std(s))
        wr = post_std / prior_std if prior_std > 0 else np.nan
        info_bits = float(0.5 * np.log((prior_std ** 2) / (post_std ** 2 + 1e-12)) / np.log(2)) \
            if post_std > 0 else np.inf
        marginal_constrained = wr < width_ratio_thresh
        deg = degenerate[p.name]
        ...
        # railed if the median sits within 10% of the prior span from either bound.
        span = p.hi - p.lo
        med = float(np.median(s))
        railed = bool((med - p.lo) < rail_frac * span or (p.hi - med) < rail_frac * span)
        # identified only if the MARGINAL is constrained, NOT degenerate with
        # another actor, and NOT railed against a prior boundary.
        identified = bool(marginal_constrained and not deg and not railed)
        report[p.name] = dict(
            median=float(np.median(s)),
            ci68=[float(np.percentile(s, 16)), float(np.percentile(s, 84))],
            ci95=[float(np.percentile(s, 2.5)), float(np.percentile(s, 97.5))],
            post_std=post_std, prior_std=float(prior_std),
            width_ratio=float(wr), info_gain_bits=info_bits,
            marginal_constrained=bool(marginal_constrained),
            degenerate_with=deg, railed=railed, identified=identified,
            units=p.units)
    return report
...
def summarize(result):
    ...
    ident = identifiability(result["samples"], result["params"])
    out = dict(engine=result["engine"], identifiability=ident)
    if "logz" in result:
        out["logz"] = result["logz"]; out["logz_err"] = result.get("logz_err")
...
# ------------------------------------------------------------- SBI seam ------
def fit_sbi(*args, **kwargs):
    """Amortized neural posterior (SBI) seam -- NOT built this sprint.

    The exact nested-sampling + MCMC engines above cover the current workload on
    CPU. When throughput demands amortization (many videos, real-time), an SBI
    posterior (sbi package, SNPE) drops in here behind the same call signature:
    train once on forward-model simulations, then infer per-video in milliseconds.
    """
    raise NotImplementedError(
        "SBI amortized posterior is a documented seam; nested sampling + MCMC "
        "are the primary engines this sprint (cheap forward model, CPU-only box).")
